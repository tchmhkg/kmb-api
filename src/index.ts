...
export default class Kmb {
    ...
    public constructor(
        ...
        const kmb = this;
        this.Stop = class {
            public constructor(public readonly id : string, name? : string, name_zh? : string, name_en?: string) {
            ...
            public get nameZh() : string | undefined {
                return stopStorage === undefined ? undefined : stopStorage.getItem(`${this.id}_zh-hant`)
                    ?? undefined;
            }
            ...
            public get nameEn() : string | undefined {
                return stopStorage === undefined ? undefined : stopStorage.getItem(`${this.id}_en`)
                    ?? undefined;
            }
                    ...
                    return result.map(
                        item => {
                            ...
                            return new kmb.Stopping(
                                new kmb.Stop(item.stop.id, name, nameZh, nameEn)
                                , new kmb.Variant(
                                    new kmb.Route(item.variant.route.number, item.variant.route.bound)
                                    , item.variant.serviceType
                                    , item.variant.origin
                                    , item.variant.originZh
                                    , item.variant.originEn
                                    , item.variant.destination
                                    , item.variant.destinationZh
                                    , item.variant.destinationEn
                                    , item.variant.description
                                    , item.variant.descriptionZh
                                    , item.variant.descriptionEn
                                )
                                , item.direction
                                , item.sequence
                                , item.fare
            ...
            public async getVariants() : Promise<Variant[]> {
                ...
                return json.data.routes.map(
                    item => new kmb.Variant(
                        ...
                        , Kmb.toTitleCase(
                            Kmb.convertHkscs(
                                item['Origin_CHI'] as keyof typeof json.data.routes[0]
                            )
                        )
                        , Kmb.toTitleCase(
                            Kmb.convertHkscs(
                                item['Origin_ENG'] as keyof typeof json.data.routes[0]
                            )
                        )
                        , Kmb.toTitleCase(
                            Kmb.convertHkscs(
                        ...
                        , Kmb.toTitleCase(
                            Kmb.convertHkscs(
                                item['Destination_CHI'] as keyof typeof json.data.routes[0]
                            )
                        )
                        , Kmb.toTitleCase(
                            Kmb.convertHkscs(
                                item['Destination_ENG'] as keyof typeof json.data.routes[0]
                            )
                        )
                        , Kmb.convertHkscs(
                            item[
                        ...
                        , Kmb.convertHkscs(
                            item['Desc_CHI'] as keyof typeof json.data.routes[0]
                        )
                        , Kmb.convertHkscs(
                            item['Desc_ENG'] as keyof typeof json.data.routes[0]
                        )
                    )
                );
            }
        ...
        this.Variant = class {
            ...
            constructor(
                ...
                , public readonly originZh : string
                , public readonly originEn : string
                , public readonly destination : string
                , public readonly destinationZh : string
                , public readonly destinationEn : string
                , public readonly description : string
                , public readonly descriptionZh : string
                ...
                return json.data.routeStops.map(
                    item => new kmb.Stopping(
                        new kmb.Stop(
                            ...
                            , Kmb.toTitleCase(
                                Kmb.convertHkscs(
                                    item['CName'] as keyof typeof item
                                )
                            )
                            , Kmb.toTitleCase(
                                Kmb.convertHkscs(
                                    item['EName'] as keyof typeof item
                                )
                            )
                        )
                        , this
    ...
    rejectUnauthorized: false
                    ...
                    : Axios.get(
                        `${kmb.corsProxyUrl ?? ''}https://etav3.kmb.hk/?action=geteta`,
                        {params : query, responseType : 'json', httpsAgent: new https.Agent({  
    rejectUnauthorized: false
  })}
                    );
                return promise.then(
            ...
            public callWebEtaApi() : Promise<EtaData[]> {
                ...
                return Axios.post(
                    ...
                    , {responseType : 'json', httpsAgent: new https.Agent({  
    rejectUnauthorized: false
  }), headers : {Origin : 'https://search.kmb.hk'}}
                ).then(
